...
const PromptBox = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Case 1: continue response if allowed and no input
    if (!trimmed && canContinueResponse && onContinueResponse) {
      onContinueResponse();
      return;
    }

    // Case 2: submit prompt
    if (trimmed) {
      onSubmitPrompt(trimmed);
      console.log("🚀 Sending prompt:", trimmed);
      setInput("");
  ...
  return (
    ...
    >
      ...
      <button
        ...
        onClick={
          isStreaming ? onCancelResponse : undefined // submit handled by `handleSubmit`
        }
        className={`${
          isStreaming ? "bg-[#9747FF]" : "bg-[#441c9a]"
        } p-2 mr-2 rounded-full hover:bg-[#3a0ca3] transition-all`}
        title={
          isStreaming
            ? "Stop response"
            : canContinueResponse && input.trim() === ""
            ? "Continue response"
            : "Send prompt"
        }
      >
